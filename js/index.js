...
var App = {
        ...
        $.each(sprites, function(index, sprite) {
            ...
            for(var i = leftN; i <= rightN; i++) {
                if(sprite.imgs[i - leftN]){
                    ...
                    .position((Math.sin(rya) * r).toFixed(5), sprite.y + sprite.h / 2 - rect.h / 2, (Math.cos(rya) * r).toFixed(5))
                    .rotation(0, ry + 180, 0)
                    .material({
    ...
    _panoHandle: function(){
        ...
        function actiondh() {
            ...
            Math.abs(o) < .1 ? panoSprites.rotationY = t : panoSprites.rotationY += .25 * o, Math.abs(r) < .1 ? panoSprites.rotationX = i : panoSprites.rotationX += .15 * r, panoSprites.updateT();
                
                               
            var s12 = -150 - 20 * Math.abs(n);
            container.z += .1 * (s12 - container.z), 
            container.updateT(),
